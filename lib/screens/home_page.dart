...
class _HomePageState extends State<HomePage> {
  ...
  bool isHeaderCollapsed = false; // Üst arayüzün küçültülüp küçültülmediğini kontrol eder
  ...
  Widget _buildHomeFeed(List<Map<String, dynamic>> homeFeedPosts) {
    ...
    return RefreshIndicator(
      onRefresh: () async {
        // Provider'dan veri yenileme
        final davaProvider = Provider.of<DavaProvider>(context, listen: false);
        await davaProvider.refreshAll();
      },
      child: ListView.builder(
        itemCount: homeFeedPosts.length,
        itemBuilder: (context, index) {
          final post = homeFeedPosts[index];
          final davaProvider = Provider.of<DavaProvider>(context, listen: false);
          final postId = post['id']?.toString() ?? 'post_$index';
          return KeyedSubtree(
            key: ValueKey(postId),
            child: _buildHomeFeedPost(post, davaProvider),
          );
        },
      ),
    );
  ...
  Widget _buildHomeFeedPost(Map<String, dynamic> post, DavaProvider davaProvider) {
    ...
    if (type == 'user_post') {
      ...
      return TwitterPostCard(
        ...
        onLike: () async {
          ...
          if (currentLiked) {
            updatedPost['payload']['likes'] = (updatedPost['payload']['likes'] ?? 1) - 1;
            updatedPost['payload']['userLiked'] = false;
          } else {
            updatedPost['payload']['likes'] = (updatedPost['payload']['likes'] ?? 0) + 1;
        ...
        onRetweet: () async {
          final updatedPost = Map<String, dynamic>.from(post);
          final currentRetweeted = updatedPost['payload']['userRetweeted'] ?? false;
          
          if (currentRetweeted) {
            updatedPost['payload']['retweets'] = (updatedPost['payload']['retweets'] ?? 1) - 1;
            updatedPost['payload']['userRetweeted'] = false;
          } else {
            updatedPost['payload']['retweets'] = (updatedPost['payload']['retweets'] ?? 0) + 1;
      ...
      final davaData = <String, dynamic>{
        ...
        'profilResmi': payload['profilResmi'] ?? 'lib/icons/07_profil_picture_davaci.png',
        'mevkii': payload['mevkii']?.toString() ?? 'Katılımcı',
        'userEmail': payload['userEmail'] ?? authorEmail,
      };
      
      // FiveCardCaseInformation widget'ını kullan (Dismissible ile sarmalı)
      return Dismissible(
        key: Key('dava_share_${post['id']}'),
        direction: DismissDirection.endToStart, // Sağdan sola (sola doğru çek)
          ...
          return await showDialog<bool>(
            ...
            builder: (BuildContext context) {
              return AlertDialog(
                ...
                content: Text('Bu davayı seyir defterinizden kaldırmak istediğinize emin misiniz?\n\n"${davaData['davaAdi'] ?? 'Dava'}"'),
                actions: [
                  TextButton(
        ...
        onDismissed: (direction) async {
          // Kullanıcının seyir defterinden bu davayı kaldır
          if (widget.userEmail != null) {
            try {
              // Home feed'den bu postu kaldır
              await davaProvider.removeHomeFeedPost(post['id']);
              
              if (context.mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text('✅ "${davaData['davaAdi'] ?? 'Dava'}" seyir defterinizden kaldırıldı'),
        ...
        child: katildigim_page.FiveCardCaseInformation(
          davaData: davaData,
          userEmail: widget.userEmail,
          isSeyirDefteri: true, // Seyir defterinde kullanıldığını belirt
          onRefresh: () async {
            // Seyir defterini yenile
            await davaProvider.refreshAll();
          },
          onDelete: () async {
            // Manuel silme (buton ile)
            final davaId = davaData['id'] as String? ?? davaData['davaId'] as String? ?? '';
            if (widget.userEmail != null && davaId.isNotEmpty) {
              try {
                await davaProvider.removeHomeFeedPost(post['id']);
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
        ...
        child: ModernDavaCard(
          ...
          onSave: () async {
            ...
            if (davaId != null && widget.userEmail != null) {
              ...
              updatedPost['payload']['finishedAt'] = DateTime.now().toIso8601String();
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text('✅ Paylaşım bitirildi: ${payload['davaAdi'] ?? 'Dava'}'),
          ...
          onYorum: (yorumMetni,
            ...
            if (isGizliTanik) {
              final gizliCount = existingComments
                  .where((comment) => comment['isGizliTanik'] == true)
                  .length;
              userName = 'GizliTanık-${gizliCount + 1}';
            ...
            try {
              if (davaId != null && widget.userEmail != null) {
                ...
                final invitationIndex = invitations.indexWhere((inv) => 
                  (inv['davaId']?.toString() ?? inv['id']?.toString()) == davaId.toString()
                );
                ...
                if (invitationIndex != -1) {
                  ...
                  HiveDatabaseService.addInvitation(widget.userEmail!, invitation);
          ...
          onBegeni: () async {
            ...
            final updatedPost = Map<String, dynamic>.from(post);
            final currentLiked = updatedPost['payload']['userLiked'] ?? false;
            final currentDisliked = updatedPost['payload']['userDisliked'] ?? false;
            ...
            try {
              ...
              if (davaId != null && widget.userEmail != null) {
                ...
                if (invitationIndex != -1) {
                  final invitation = Map<String, dynamic>.from(invitations[invitationIndex]);
                  invitation['begeniSayisi'] = updatedPost['payload']['begeniSayisi'];
                  invitation['begenmemeSayisi'] = updatedPost['payload']['begenmemeSayisi'];
                  invitation['userLiked'] = updatedPost['payload']['userLiked'];
                  invitation['userDisliked'] = updatedPost['payload']['userDisliked'];
  ...
  Widget _buildDavaWatchCard(
    ...
    final String rawState = (payload['watchState'] ?? '').toString().toLowerCase();
    final bool isWatching = rawState == 'on' || payload['isWatching'] == true;
    final String fallbackLabel = isWatching
        ? 'ON'
        : (rawState.isNotEmpty ? rawState.substring(0, rawState.length >= 2 ? 2 : rawState.length).toUpperCase() : 'OF');
    final String stateLabel = (payload['watchStateLabel'] ?? fallbackLabel).toString();
    final String caseName = (payload['davaAdi'] ?? 'Dava Adı Belirtilmemiş').toString();
    ...
    final Color badgeFill = isWatching ? const Color(0xFFE8F5E9) : Colors.grey.shade100;
    final Color badgeBorder = isWatching ? const Color(0xFF66BB6A) : Colors.grey.shade300;
    final Color stateTextColor = isWatching ? const Color(0xFF2E7D32) : Colors.grey.shade600;
    // Kullanıcı email'ini al ve profil resmini Hive'dan oku
    final String? userEmail = payload['userEmail']?.toString() ?? widget.userEmail;
        ...
        children: [
          ...
          InkWell(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: isWatching ? const Color(0xFFFFF3E0) : Colors.grey.shade200,
                borderRadius: BorderRadius.circular(8),
                border: Border.all(
                  color: isWatching ? const Color(0xFFFFB74D) : Colors.grey.shade400,
                  width: 1.2,
                ),
              ),
              child: Text(
  ...
  Future<void> _handleWatchCardToggle(
    ...
    final bool isCurrentlyWatching =
        (payload['watchState']?.toString().toLowerCase() == 'on') || (payload['isWatching'] == true);
            ...
            return Container(
              ...
              child: SingleChildScrollView(
                ...
                child: IlgililerinSeyirDefteriWidgeti(
                  ...
                  davaci: payload['davaci']?.toString(),
                  davali: payload['davali']?.toString(),
                  kategori: payload['kategori']?.toString() ?? payload['davaKategori']?.toString(),
  ...
  void _showActigimDavalarDialog() {
    ...
    final List<Map<String, dynamic>> openedDavalar = HiveDatabaseService.getOpenedDavalar();
    final List<Map<String, dynamic>> savedDavalar = HiveDatabaseService.getSavedDavalar();
    
    // Tüm davaları birleştir
    final List<Map<String, dynamic>> allDavalar = [...openedDavalar, ...savedDavalar];
  ...
  Widget _buildDavalarList(BuildContext context, List<Map<String, dynamic>> davalar) {
    return Container(
      padding: const EdgeInsets.all(16),
      child: ListView.builder(
          ...
          return Container(
            ...
            decoration: BoxDecoration(
              ...
              gradient: LinearGradient(
                ...
                colors: [
                  Colors.white,
                  isOpened ? Colors.green.withOpacity(0.02) : Colors.orange.withOpacity(0.02),
                ],
              ),
              boxShadow: [
                BoxShadow(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: Icon(
                              ...
                              color: isOpened ? Colors.green.shade600 : Colors.orange.shade600,
                              size: 20,
                            ),
                          ),
                          const SizedBox(width: 12),
                      ...
                      Container(
                        ...
                        child: Column(
                          children: [
                            ...
                            if ((dava['davaKategori']?.toString().isNotEmpty == true) ||
                                (dava['kategori']?.toString().isNotEmpty == true) ||
                                (dava['davaKategorisi']?.toString().isNotEmpty == true))
                              _buildDetailRow(
                                '📋 Kategori', 
      ...
      floatingActionButton: authProvider.isAdmin
          ? FloatingActionButton(
              onPressed: () {
                Navigator.of(context).push(
                  MaterialPageRoute(
                    builder: (context) => AdminPage(adminEmail: widget.userEmail!),
                  ),
                );
              },
                      ...
                      children: [
                        ...
                        Padding(
                          ...
                          child: dava_ac.TreeMenuPageheadlines(
                            title: "SEYİR DEFTERİ", // HomePage için özel başlık
              ...
              child: Row(
                ...
                children: [
                              ...
                              children: [
                                ...
                                GestureDetector(
                                  onTap: () {
                                    Navigator.push(
                                      context,
                                      MaterialPageRoute(builder: (context) => katildigim_page.KatildigimDavalarPage(userEmail: widget.userEmail)),
                                    );
                                  },
                                  child: Padding(
                                    padding: const EdgeInsets.fromLTRB(8.0, 48.0, 8.0, 8.0),
                                ...
                                GestureDetector(
                                  onTap: () {
                                    Navigator.push(
                                      ...
                                      MaterialPageRoute(
                                        builder: (context) =>
                                            TrendInsightsPage(userEmail: widget.userEmail),
                                      ),
                                    );
                                  },
                                  child: Padding(
                                ...
                                GestureDetector(
                                  onTap: () {
                                    Navigator.push(
                                      ...
                                      MaterialPageRoute(
                                        builder: (context) =>
                                            SavedHaykirlarPage(userEmail: widget.userEmail),
                                      ),
                                    );
                                  },
                                  child: Padding(
                                  ...
                                  onTap: () {
                                    Navigator.push(
                                      ...
                                      MaterialPageRoute(
                                        builder: (context) =>
                                            StatisticsDashboardPage(userEmail: widget.userEmail),
                                      ),
                                    );
                                  },
                                  child: Padding(
                                ...
                                GestureDetector(
                                  onTap: () {
                                    Navigator.push(
                                      context,
                                      MaterialPageRoute(
                                        builder: (context) => HesapGizlilikAyarlariPage(userEmail: widget.userEmail),
                                      ),
                                    );
                                  },
                                  child: Padding(
                    ...
                    Expanded(
                      child: Column(
                        children: [
                          // Twitter/X benzeri post paylaşım alanı
                          TwitterPostComposer(userEmail: widget.userEmail),
  ...
  Widget _buildHaykirCard(
    ...
    DavaProvider davaProvider,
      ...
      final postId = post['id']?.toString() ?? 'haykir_${DateTime.now().millisecondsSinceEpoch}';
...
class _HaykirCardWidgetState extends State<HaykirCardWidget> with TickerProviderStateMixin {
  ...
  late AnimationController _hourglassController; // ✅ Hareketli kum saati animasyonu
  late AnimationController _expandController; // ✅ Açılma/kapanma animasyonu
  AnimationController? _groupIconController; // ✅ Grup ikonu animasyonu (nullable - hot reload için)
  ...
  void dispose() {
    ...
    _groupIconController?.dispose(); // ✅ Dispose (nullable check - hot reload için)
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final createdAt = widget.payload['createdAt']?.toString() ?? widget.post['createdAt']?.toString() ?? '';
    ...
    final commentCount = stats['commentCount'] as int? ?? widget.payload['commentCount'] as int? ?? 0;
    final retweetCount = stats['retweetCount'] as int? ?? widget.payload['retweetCount'] as int? ?? 0;
    final likeCount = stats['likeCount'] as int? ?? widget.payload['likeCount'] as int? ?? 0;
    final kinaCount = stats['kinaCount'] as int? ?? widget.payload['kinaCount'] as int? ?? 0;
    final isLiked = stats['isLiked'] as bool? ?? widget.payload['isLiked'] as bool? ?? false;
    final isSaved = stats['isSaved'] as bool? ?? widget.payload['isSaved'] as bool? ?? false;
    final isKina = stats['isKina'] as bool? ?? widget.payload['isKina'] as bool? ?? false; // ✅ Kına durumu
    final isRetweeted = stats['isRetweeted'] as bool? ?? false; // ✅ Retweet durumu
    final isCommented = stats['isCommented'] as bool? ?? false; // ✅ Yorum durumu
    
    // ✅ Haykır süresi: 19 gün
    const int totalDays = 19;
    const int totalHours = totalDays * 24; // 456 saat
    String kalanSure = '$totalDays gün';
    ...
    if (createdAt.isNotEmpty) {
      try {
        ...
        final remainingHours = totalHours - difference.inHours;
        
        if (remainingHours <= 0) {
          kalanSure = 'Süre doldu';
          isExpired = true;
          
          // ✅ 19 gün sonunda puanlama kontrolü (sadece bir kez)
          if (!_scoringChecked) {
            _scoringChecked = true;
            _checkAndApplyHaykirScoring(haykirId, likeCount, kinaCount, createdAt).then((_) {
              if (mounted) {
                setState(() {}); // ✅ Widget'ı yeniden build et ki badge görünsün
              }
            });
          }
        } else {
          final remainingDays = remainingHours ~/ 24;
          final remainingHoursInDay = remainingHours % 24;
          kalanSure = '$remainingDays gün $remainingHoursInDay saat';
        }
      } catch (e) {
        kalanSure = '$totalDays gün';
      }
    }
    ...
    if (widget.userEmail != null && widget.userEmail!.isNotEmpty) {
      final katildigimHaykirler = HiveDatabaseService.getKatildigimHaykirler(widget.userEmail!);
    ...
    final haykirData = HiveDatabaseService.getHaykir(haykirId);
    final scoringApplied = haykirData?['scoringApplied'] == 'true' || haykirData?['scoringApplied'] == true;
    final isSuccessStr = haykirData?['isSuccess']?.toString();
    final isSuccess = isSuccessStr == 'true';
    final showSuccessBadge = isExpired && scoringApplied && isSuccessStr != null;
                    ...
                    children: [
                      ...
                      if (widget.showCloseButton)
                        IconButton(
                          ...
                          onPressed: () async {
                            await widget.davaProvider.removeHomeFeedPost(widget.post['id']);
                            if (mounted) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  content: Text('✅ Haykır seyir defterinden kaldırıldı'),
                                  duration: Duration(seconds: 2),
                                ),
                              );
                            }
              ...
              child: Column(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        Icon(MdiIcons.formatQuoteOpen, size: 30, color: Colors.blue.shade600),
                        const SizedBox(width: 8),
                        Expanded(
                          child: Text(
                            slogan,
                  ...
                  // ✅ Direme Alanı
                  Row(
                    children: [const SizedBox(width: 36),
                      ...
                      GestureDetector(
                        onTap: isGroupIconDisabled ? null : () async {
                            ...
                            : _groupIconController != null
                                ? AnimatedBuilder(
                                    ...
                                    builder: (context, child) {
                                      ...
                                      final opacity = 0.2 + (0.8 * (0.5 + 0.5 * (math.sin(animationValue * 2 * math.pi))));
                                      // ✅ Pulse efekti için scale (1.0 - 1.2 arası)
                                      final scale = 1.0 + (animationValue * 0.2);
                                        ...
                                        child: Transform.scale(
                                          ...
                                          child: Container(
                                            ...
                                            decoration: BoxDecoration(
                                              ...
                                              boxShadow: [
                                                BoxShadow(
                                                  color: Colors.purple.withOpacity(0.2 + (animationValue * 0.4)),
                                                  blurRadius: 6 + (animationValue * 6),
                                                  spreadRadius: 1 + (animationValue * 3),
                                                ),
                                              ],
                                            ),
                                            child: Icon(
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      Icon(
                        showSuccessBadge
                            ? (isSuccess ? MdiIcons.thumbUp : Icons.thumb_down)
                            : MdiIcons.chartLine,
                        color: showSuccessBadge
                            ? (isSuccess ? Colors.yellow[800] : Colors.red[700])
                      ...
                      Flexible(
                        child: Text(
                          showSuccessBadge
                              ? (isSuccess ? 'ETKİLEŞİM BAŞARILI' : 'ETKİLEŞİM BAŞARISIZ')
                              : 'ETKİLEŞİM',
                          style: TextStyle(
                            fontSize: 12,
                            fontWeight: FontWeight.bold,
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: isExpired 
                              ? (showSuccessBadge 
                                  ? (isSuccess ? Colors.yellow[800] : Colors.grey.shade600) // ✅ Başarılı: beğen rengi (kırmızı), Başarısız: kına rengi (gri)
                                  : Colors.grey.shade600) // ✅ Henüz puanlama yapılmadıysa gri
                              : Colors.orange.shade500, // ✅ Aktif durum: turuncu
                          borderRadius: BorderRadius.circular(20),
                          boxShadow: [
                            BoxShadow(
                              color: isExpired
                                  ? (showSuccessBadge
                                      ? (isSuccess ? Colors.red : Colors.grey).withOpacity(0.3) // ✅ Başarılı: kırmızı shadow, Başarısız: gri shadow
                                      : Colors.grey.withOpacity(0.3))
                                  : Colors.orange.withOpacity(0.3), // ✅ Aktif durum: turuncu shadow
                              blurRadius: 6,
                              spreadRadius: 0.5,
                              offset: const Offset(0, 2),
                            ),
                  ...
                  Row(
                    ...
                    children: [
                      _buildSocialIconButton(
                        ...
                        count: commentCount,
                        color: Colors.green,
                        isActive: isCommented || showComments, // ✅ Kullanıcı yorum yazdıysa veya yorumlar açıksa aktif
                        onTap: () {
                          setState(() {
                      ...
                      _buildSocialIconButton(
                        ...
                        count: retweetCount,
                        color: Colors.orange,
                        isActive: isRetweeted, // ✅ Kullanıcı retweet yaptıysa aktif
                        isDisabled: _isRetweetDisabled(haykirId) || _isRetweetPermanent(haykirId),
                        onTap: (_isRetweetDisabled(haykirId) || _isRetweetPermanent(haykirId))
                            ? null 
                            : () {
                      ...
                      _buildSocialIconButton(
                        ...
                        color: Colors.red,
                        isActive: isLiked,
                        isDisabled: isExpired, // ✅ Süre dolduğunda devre dışı
                        onTap: isExpired ? null : () async {
                          ...
                          updatedPost['payload']['isLiked'] = newIsLiked;
                          updatedPost['payload']['likeCount'] = newIsLiked ? likeCount + 1 : (likeCount > 0 ? likeCount - 1 : 0);
                          updatedPost['payload']['isKina'] = newIsLiked ? false : isKina; // ✅ Kına'yı kaldır
                          await widget.davaProvider.updateHomeFeedPost(widget.post['id'], updatedPost);
                        },
                      ),
                      _buildSocialIconButton(
                        ...
                        onTap: isExpired ? null : () async {
                          ...
                          final updatedPost = Map<String, dynamic>.from(widget.post);
                          updatedPost['payload']['isKina'] = newIsKina;
                          updatedPost['payload']['kinaCount'] = newIsKina ? kinaCount + 1 : (kinaCount > 0 ? kinaCount - 1 : 0);
                          updatedPost['payload']['isLiked'] = newIsKina ? false : isLiked; // ✅ Beğen'i kaldır
                          updatedPost['payload']['likeCount'] = newIsKina && isLiked ? (likeCount > 0 ? likeCount - 1 : 0) : likeCount;
                          await widget.davaProvider.updateHomeFeedPost(widget.post['id'], updatedPost);
                        },
                      ),
                      ...
                      _buildSocialIconButton(
                        ...
                        onTap: () async {
                          ...
                          // ✅ Kaydet basınca haykirislarim_page'e ekle
                          if (newIsSaved) {
                            try {
                              final haykirData = HiveDatabaseService.getHaykir(haykirId);
                              if (haykirData != null) {
                                // HaykirislarimPage için kaydetme işlemi burada yapılacak
                                print('✅ Haykır kaydedildi (haykirislarim_page için): $haykirId');
                              }
                            } catch (e) {
                          ...
                          // ✅ Kaydet butonuna basınca widget'ı da kaydedilenler arşivine ekle
                          if (widget.userEmail != null && widget.userEmail!.isNotEmpty) {
                            // Widget ID'si her zaman aynı olmalı (icon durumuna göre değişmez)
                            const widgetId = 'home_page_Kaydet_bookmark';
                            final currentIcon = newIsSaved ? MdiIcons.bookmark : MdiIcons.bookmarkOutline;
                            
                            if (newIsSaved) {
                              // Widget'ı kaydet (güncelle veya ekle)
                              HiveDatabaseService.saveWidget(
                                userEmail: widget.userEmail!,
                                widgetId: widgetId,
                                label: 'Kaydet',
                                iconCodePoint: currentIcon.codePoint.toString(),
                              ...
                              // Kayıt kaldırıldığında widget'ı da kaldır
                          ...
                          final updatedPost = Map<String, dynamic>.from(widget.post);
                          updatedPost['payload']['isSaved'] = newIsSaved;
                          await widget.davaProvider.updateHomeFeedPost(widget.post['id'], updatedPost);
  ...
  Future<void> _onGroupIconTap(String haykirId) async {
    ...
    try {
      ...
      final katildigimHaykir = {
        ...
        'kalanSure': '19 gün', // Varsayılan süre
        'profilResmi': 'lib/icons/03_haykir_ana_icon.png',
      };

      // Katıldığım haykırları kaydet
      await HiveDatabaseService.addKatildigimHaykir(widget.userEmail!, katildigimHaykir);
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('✅ Haykıra katıldınız! "KATILDIĞIM" sekmesinde görünecek.'),
            duration: const Duration(seconds: 3),
            backgroundColor: Colors.purple,
  ...
  Future<void> _processRetweet(String haykirId, String selectedOption) async {
    ...
    try {
      ...
      if (selectedOption == 'grup19') {
        ...
        print('🔍 DEBUG: widget.post[\'authorEmail\']: ${widget.post['authorEmail']}');
        print('🔍 DEBUG: widget.payload[\'userEmail\']: ${widget.payload['userEmail']}');
        print('🔍 DEBUG: haykirData[\'userEmail\']: ${haykirData['userEmail']}');
        print('🔍 DEBUG: Bulunan authorEmail: $authorEmail');
        print('🔍 DEBUG: Davet gönderen kişi (widget.userEmail): ${widget.userEmail}');
        ...
        if (authorEmail.isNotEmpty && authorEmail != widget.userEmail) {
          ...
          print('🔍 DEBUG: Haykır başlatan kişi Grup19 listesinde mi? $authorAlreadyInList');
          ...
          if (authorEmail.isEmpty) {
            ...
            print('ℹ️ Haykır başlatan kişi davet gönderen kişi ile aynı, davet gönderilmedi.');
          }
        }
      ...
      final updatedPost = Map<String, dynamic>.from(widget.post);
      final currentRetweetCount = updatedPost['payload']['retweetCount'] ?? 0;
      updatedPost['payload']['retweetCount'] = currentRetweetCount + 1;
      updatedPost['payload']['isRetweeted'] = true;
      await widget.davaProvider.updateHomeFeedPost(widget.post['id'], updatedPost);

      // ✅ State'i güncelle
      if (mounted) {
        setState(() {
          // State güncellenmesi için widget yeniden build edilecek
        });
      }
    } catch (e) {
  ...
  Future<void> _undoRetweet(String haykirId) async {
    ...
    try {
      ...
      final updatedPost = Map<String, dynamic>.from(widget.post);
      final currentRetweetCount = updatedPost['payload']['retweetCount'] ?? 0;
      updatedPost['payload']['retweetCount'] = currentRetweetCount > 0 ? currentRetweetCount - 1 : 0;
      updatedPost['payload']['isRetweeted'] = false;
      await widget.davaProvider.updateHomeFeedPost(widget.post['id'], updatedPost);
      
      // ✅ Adım 4: State'i güncelle
      if (mounted) {
        setState(() {
          // State güncellenmesi için widget yeniden build edilecek
        });
  ...
  Future<void> _checkAndApplyHaykirScoring(
    ...
    try {
      ...
      if (haykirData['scoringApplied'] == 'true' || haykirData['scoringApplied'] == true) {
        return;
      }
      ...
      if (destekDouble >= minDestek) {
        if (destekDouble > kinaMa) {
          ...
          isSuccess = true;
          await _addUserPoints(authorEmail, 19, 'Haykır başarılı (DESTEK > KINAma)');
          print('✅ Haykır başarılı: $authorEmail +19 puan aldı (DESTEK: $destek, KINAma: $kinaMa)');
        } else {
          // ✅ DESTEK >= 6.859 AMA DESTEK < KINAma → Haykır LİDERi -19 puan
          // Not: "Haykır LİDERi" = Haykır başlatan kişi (aynı kişi)
          isSuccess = false;
          await _addUserPoints(authorEmail, -19, 'Haykır başarısız (DESTEK < KINAma)');
          print('❌ Haykır başarısız: $authorEmail -19 puan aldı (DESTEK: $destek, KINAma: $kinaMa)');
        }
      } else {
      ...
      await HiveDatabaseService.updateHaykir(haykirId, {
        ...
        'isSuccess': isSuccess.toString(), // ✅ Başarı durumu: 'true' veya 'false'
      });
    } catch (e) {
  ...
  Future<void> _addUserPoints(String userEmail, int points, String reason) async {
    try {
      ...
      if (existingData != null) {
        ...
        currentPoints = int.tryParse(data['totalPoints']?.toString() ?? '0') ?? 0;
      ...
      print('✅ Kullanıcı puanı güncellendi: $userEmail -> $newPoints (${points > 0 ? '+' : ''}$points)');
    } catch (e) {
            ...
            if (widget.userEmail != null && widget.userEmail!.isNotEmpty) {
              ...
              if (isAlreadySaved) {
                ...
                HiveDatabaseService.saveWidget(
                  ...
                  additionalData: {
                    'postId': widget.post['id']?.toString() ?? '',
                    'haykirId': widget.post['payload']?['haykirId']?.toString() ?? '',
                  },
                );
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                  ...
                  style: TextStyle(
                    ...
                    color: isDisabled
                        ...
                        : (isActive && isKinaButton 
                            ...
                            : (isActive ? color : Colors.grey[600])),
                    fontWeight: isActive && !isDisabled ? FontWeight.bold : FontWeight.w600,
                    letterSpacing: 0.5,
                    decoration: isDisabled ? TextDecoration.lineThrough : null, // ✅ Devre dışı durumda üstü çizili
                  ),
                  textAlign: TextAlign.center,
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double i = 0; i <= size.width; i += 1) {
      final y = size.height * 0.25 + 
                (size.height * 0.1) * 
                math.sin((i / size.width * 3 * math.pi) + (animationValue * 2 * math.pi));
    ...
    for (double i = 0; i <= size.width; i += 1) {
      final y = size.height * 0.75 + 
                (size.height * 0.1) * 
                math.sin((i / size.width * 3 * math.pi) + (animationValue * 2 * math.pi) + math.pi);
